...
const getOneHandler = async (req, res) => {
  ...
  if (!distributeur) {
    return res
      .status(404)
      .json({ status: "Not Found", message: "Distributeur not found" });
  }
  return res.status(200).json({ status: "success", data: distributeur });
};

const postHandler = async (req, res) => {
  const { etat, type, codeDeverouillage, position, idClient, idRegion, idAM } =
    req.body;
  const newDistributeur = await createDistributeur(
    etat,
    type,
    codeDeverouillage,
    position,
...
const putHandler = async (req, res) => {
  ...
  if (!updatedDistributeur) {
    return res
      ...
      .json({
        status: "Bad Request",
        message: "provided costumer is not valid",
      });
  }
